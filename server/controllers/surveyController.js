...
import { sqliteDateTimeAsUtcIso } from '../utils/dates.js'
...
function validateImageUrl(v) {
  ...
  if (s === '') return null
  // Only allow relative uploads paths served by this server
...
function mapSurveyRow(row, extra = {}) {
  return {
    ...
    status: row.status,
    activeFromDate:
      row.activeFromDate != null ? sqliteDateTimeAsUtcIso(row.activeFromDate) : row.activeFromDate,
    activeToDate:
      row.activeToDate != null ? sqliteDateTimeAsUtcIso(row.activeToDate) : row.activeToDate,
    aiReviewScore: row.aiReviewScore,
    isReviewed: Boolean(row.isReviewed),
    createdDate: sqliteDateTimeAsUtcIso(row.createdDate),
    ...extra,
  }
}
...
export async function listSurveys(req, res) {
  ...
  if (filter === 'unresponded') {
    if (userId) {
      where += ` AND s.createdByUserId != ? AND NOT EXISTS (
      SELECT 1 FROM responses r WHERE r.surveyId = s.id AND r.respondentUserId = ?
    )`
  ...
  const afterClause = cursor ? 'AND s.id < ?' : ''
  if (cursor) params.push(cursor)

  let orderBy = 's.id DESC'
  if (sort === 'votes') {
    orderBy = 'vote_count DESC, s.id DESC'
  } else if (sort === 'latest') {
    orderBy = 's.createdDate DESC, s.id DESC'
  }

  const countSql = `SELECT COUNT(*) AS total FROM surveys s WHERE ${where}`
  ...
  let sql = `
      ...
      ${userId ? `, (SELECT EXISTS (SELECT 1 FROM responses r2 WHERE r2.surveyId = s.id AND r2.respondentUserId = ?)) AS user_has_responded` : ''}
      FROM surveys s
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    if (Array.isArray(q.options) && ['single', 'multiple', 'dropdown'].includes(q.questionType)) {
      for (let j = 0; j < q.options.length; j++) {
        ...
        if (opt) {
          await run(
            'INSERT INTO question_options (questionId, optionText, orderIndex, createdBy, updatedBy) VALUES (?, ?, ?, \'api\', \'api\')',
            [qid, String(opt), j],
          )
        }
      }
...
export async function updateSurvey(req, res) {
  ...
  // MSSQL doesn't have datetime('now') - use GETUTCDATE() or let column default handle it
  if (title != null) {
    if (DB_TYPE === 'mssql') {
      await run(
        ...
        updatedDate = SYSUTCDATETIME(), updatedBy = 'api' WHERE id = ?`,
      ...
      await run(
        `UPDATE surveys SET title = ?, description = COALESCE(?, description), imageUrl = COALESCE(?, imageUrl),
        status = COALESCE(?, status), activeFromDate = COALESCE(?, activeFromDate), activeToDate = COALESCE(?, activeToDate),
        updatedDate = datetime('now'), updatedBy = 'api' WHERE id = ?`,
        [title, description, imageUrl, status, activeFromDate, activeToDate, id],
      )
    }
  } else {
  ...
  if (isReviewed) {
    if (DB_TYPE === 'mssql') {
      await run(
        'UPDATE surveys SET isReviewed = 1, aiReviewScore = ?, reviewJson = ?, updatedDate = SYSUTCDATETIME() WHERE id = ?',
        [aiReviewScore != null ? Number(aiReviewScore) : null, reviewJson ? JSON.stringify(reviewJson) : null, id],
      )
    } else {
      await run(
        'UPDATE surveys SET isReviewed = 1, aiReviewScore = ?, reviewJson = ?, updatedDate = datetime(\'now\') WHERE id = ?',
        [aiReviewScore != null ? Number(aiReviewScore) : null, reviewJson ? JSON.stringify(reviewJson) : null, id],
      )
    }
  }
...
export async function postVote(req, res) {
  ...
  // MSSQL doesn't have INSERT OR IGNORE - use MERGE or check first
  if (DB_TYPE === 'mssql') {
    const existing = await get('SELECT 1 AS found FROM survey_votes WHERE surveyId = ? AND userId = ?', [surveyId, userId])
    if (!existing) {
      await run('INSERT INTO survey_votes (surveyId, userId) VALUES (?, ?)', [surveyId, userId])
    }
  } else {
    await run('INSERT OR IGNORE INTO survey_votes (surveyId, userId) VALUES (?, ?)', [surveyId, userId])
  }
